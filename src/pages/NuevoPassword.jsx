...
const NuevoPassword = () =>
{
    ...
    useEffect(() =>
    {
        const comprobarToken = async () =>
        {
            try 
            {
                await clientaAxios(`/veterinarios/olvide-password/${token}`);
    ...
    const handleSubmit = async e =>
    {
        ...
        try 
        {
            ...
            const { data } = await clientaAxios.post(url, {password});
    ...
    return (
        <>
            <div>
                <h1 className="text-indigo-600 font-black text-6xl">Reestablece tu Constraseña y no Pierdas Acceso a <span className="text-black">tus Pacientes</span></h1>
            </div>
            ...
            <div className='mt-20 md:mt-5 shadow-lg px-5 py-10 rounded-xl bg-white'>
                ...
                {passwordModificado && <Link to="/" className='block text-center my-5 text-gray-500'>¿Ya tienes una cuenta? <span className='font-bold'>Inicia Sesión</span></Link>}
            </div>
        </>
    )
};
